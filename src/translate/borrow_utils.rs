...
pub(super) fn drop_reference(
    ...
    match alias_groups.find_curr_name(reference_name) {
...
pub(super) fn translate_assignment<LhsType>(
    ...
    if is_borrow(rhs) {
        ...
        match rhs {
            ...
            Var(rhs_name)
                if {
                    let alias_curr_name = alias_groups
                        .find_curr_name(&rhs_name)
                        .unwrap_or_else(|| &rhs_name);
...
/// Assumes reference (curr & final) is alrady present in head query params, referenced variable as well but not with apostrophe
pub(super) fn borrow_variable(
    ...
    let referenced_var_updated = Var(referenced_name.clone() + "'");
